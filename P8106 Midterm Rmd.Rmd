...
roc.ctree <- roc(stroke_df_no_na$stroke[-trRows], ctree.pred)
roc.gbm <- roc(stroke_df_no_na$stroke[-trRows], gbm.pred)
roc.gbmA <- roc(stroke_df_no_na$stroke[-trRows], gbmA.pred)
...
Exactly the same AUC values but different curves -- is this a caret specification?
WHICH OF THE EDA PLOTS TO KEEP IN THE REPORT?
implement ROSE.
do i need to be inputting a matrix or selecting the outcome for caret?
...
-rerun model fitting
-check variable importances are the same from the extraciton method and the permutation setting are the same.