...
```{r plottingincludeds, fig.align='center', fig.width=6, echo = F}
stroke_df_no_na$stroke_labs = factor(stroke_df_no_na$stroke)
stroke_df_no_na$stroke_labs = factor(
                                      stroke_df_no_na$stroke, 
                                      levels = c("X0", "X1"), 
...
(p5 + p7 + p6) / (p9) + plot_annotation(
  ...
  subtitle = 'Boxplot of grouped distributions, over scatter plot of bmi vs glucose and colored by age',
  caption = 'Notice higher age among those with stroke outcomes, otherwise similar'
)
...
```{r corrplot, fig.align='center', fig.width=6, echo = F}
corrplot(cor(x), method = "square", type = "upper")
```
...
```{r, fig.align='center', fig.width=6, echo = F, warning = F}
ggplot(rf.fit, highlight = TRUE) +
  labs(
...
legend("bottomright", legend = paste0(modelNames, ": ", round(auc[1:4],3)), col = 1:4, lwd = 1)
```
...
legend("bottomright", legend = paste0(modelNames2, ": ", round(auc[5:8],3)), col = 1:8, lwd = 1)
```
