...
```{r modeling rf using ranger, include = F, cache = T}
#from lecture 12
set.seed(1107)
ctrl <- trainControl(method = "cv",
                     classProbs = TRUE, 
...
```{r modeling ctree, include=F, cache = T}
set.seed(1107)
...
```{r ctree plot, include=F, cache = T}
...
```{r adaboost, include=F, cache = T, message = F}
...
This application uses the random forest, conditional inference tree, and gradient boosting (and adaboost). 
...
```{r modeling 4 test performance, include = FF}
#from lecture 9
glm.pred <- predict(model.glm, newdata = stroke_df_no_na[-trRows,], type = "prob")[,2]
glmn.pred <- predict(model.glmn, newdata = dat[-trRows,], type = "prob")[,2]
knn.pred <- predict(model.knn, newdata = dat[-trRows,], type = "prob")[,2]
gam.pred <- predict(model.gam, newdata = dat[-trRows,], type = "prob")[,2]
mars.pred <- predict(model.mars, newdata = dat[-trRows,], type = "prob")[,2]
#from lecture 10
lda.pred <- predict(model.lda, newdata = dat[-trRows,], type = "prob")[,2]
nb.pred <- predict(model.nb, newdata = dat[-trRows,], type = "prob")[,2]
qda.pred <- predict(model.qda, newdata = dat[-trRows,], type = "prob")[,2]
#combined
roc.glm <- roc(dat$diabetes[-trRows], glm.pred)
roc.glmn <- roc(dat$diabetes[-trRows], glmn.pred)
roc.knn <- roc(dat$diabetes[-trRows], knn.pred)
roc.gam <- roc(dat$diabetes[-trRows], gam.pred)
roc.mars <- roc(dat$diabetes[-trRows], mars.pred)
roc.lda <- roc(dat$diabetes[-trRows], lda.pred)
roc.nb <- roc(dat$diabetes[-trRows], nb.pred)
roc.qda <- roc(dat$diabetes[-trRows], qda.pred)
#combined
...
add the make.levels to the data cleaning steps
add corrplot to eda
gender data cleaning: factor the variable again

what about:
```{r modeling 2, include = F}
#from lecture 10
#qda
model.qda <- train(x = stroke_df_no_na[trRows,2:11],
                   y = stroke_df_no_na$stroke[trRows],
                   method = "qda",
                   metric = "ROC",
                   trControl = ctrl)

model.mars <- train(x = stroke_df_no_na[trRows,3:11],
                    y = stroke_df_no_na$stroke[trRows],
                    method = "earth",
...
model.nb <- train(x = stroke_df_no_na[trRows,2:11],
                   y = stroke_df_no_na$stroke[trRows],
                  method = "nb",
                  tuneGrid = nbGrid,
                  metric = "ROC",
