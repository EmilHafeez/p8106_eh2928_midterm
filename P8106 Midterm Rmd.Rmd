...
#gbm ada
gbmA.grid.norose <- expand.grid(n.trees = c(2000,3000,4000),
                         interaction.depth = 1:6,
                         shrinkage = c(0.001,0.003,0.005),
                         n.minobsinnode = c(1,10))
set.seed(1107)
gbmA.fit.norose <- train(stroke ~ . , 
                  stroke_df_no_na, 
                  subset = trRows, 
                  tuneGrid = gbmA.grid.norose,
                  trControl = ctrl,
                  method = "gbm",
                  distribution = "adaboost",
                  metric = "ROC",
...
ggplot(gbm.fit.norose, highlight = TRUE)
ggplot(gbmA.fit.norose, highlight = TRUE)
```
...
res <- resamples(list(
                      ...
                      rf = rf.fit.norose,
                      ctree = ctree.fit.norose,
                      gbma = gbmA.fit.norose,
                      ridge = ridge.fit.norose,
                      )
                 )
summary(res)
...
#norose
rf.pred <- predict(rf.fit.rose, newdata = stroke_df_no_na[-trRows,], type = "prob")[,1]
ctree.pred <- predict(ctree.fit.rose, newdata = stroke_df_no_na[-trRows,], type = "prob")[,1]
gbmA.pred <- predict(gbmA.fit.rose, newdata = stroke_df_no_na[-trRows,], type = "prob")[,1]
ridge.pred <- predict(ridge.fit.rose, newdata = stroke_df_no_na[-trRows,], type = "prob")[,1]
...
#norose
roc.rf.norose <- roc(stroke_df_no_na$stroke[-trRows], rf.pred)
roc.ctree.norose <- roc(stroke_df_no_na$stroke[-trRows], ctree.pred)
roc.gbmA.norose <- roc(stroke_df_no_na$stroke[-trRows], gbmA.pred)
roc.ridge.norose <- roc(stroke_df_no_na$stroke[-trRows], ridge.pred)
...
plot(roc.gbmA.norose, col = 4, add = TRUE)
